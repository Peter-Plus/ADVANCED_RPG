...
FWarriorEnemyWaveSpawnerTableRow* AWarriorSurvivalGameMode::GetCurrentWaveSpawnerTableRow() const
{
	...
	FWarriorEnemyWaveSpawnerTableRow* FoundRow = 
		EnemyWaveSpawnerDataTable->FindRow<FWarriorEnemyWaveSpawnerTableRow>(RowName,FString());
...
int32 AWarriorSurvivalGameMode::TrySpawnWaveEnemies()
{
	...
	uint32 EnemiesSpawnedThisTime = 0;//本次生成敌人数（非本波次）
	...
	for (const FWarriorEnemyWaveSpawnerInfo& SpawnerInfo : GetCurrentWaveSpawnerTableRow()->EnemyWaveSpawnerDefinitions)
	{
		...
		const int32 RandomSpawnNum = FMath::RandRange(SpawnerInfo.MinPerSpawnCount,SpawnerInfo.MaxPerSpawnCount);
		...
		for (int32 i = 0; i < RandomSpawnNum; i++)
		{
			...
			RandomLocation += FVector(0.f,0.f,80.f);
			
			AWarriorEnemyCharacter* SpawnedEnemy = GetWorld()->SpawnActor<AWarriorEnemyCharacter>(
				LoadedEnemyClass,RandomLocation,SpawnRotation,SpawnParams);
...
void AWarriorSurvivalGameMode::OnEnemyDestroyed(AActor* DestroyedActor)
{
	...
	{
		// 当不应该继续生成敌人且当前波次敌人击杀完后清除缓存准备开启新波次
