...
{
	GetAvailableActorsToLock();// 刷新目标缓存
...
void UHeroGameplayAbility_TargetLock::GetAvailableActorsToLock()
{
	...
	// 获取玩家当前实际看到的镜头位置和镜头旋转
	FVector CameraLocation;
	FRotator CameraRotation;
	HeroController->GetPlayerViewPoint(CameraLocation,CameraRotation);
	
	const FVector TraceStart = HeroCharacter->GetActorLocation();
	const FVector CameraForwardDirection = CameraRotation.Vector();
	const FVector TraceEnd =TraceStart + CameraForwardDirection * BoxTraceDistance;
...
void UHeroGameplayAbility_TargetLock::GetAvailableActorsAroundTarget(TArray<AActor*>& OutActorsOnLeft, TArray<AActor*>& OutActorsOnRight)
{
	...
	for (AActor* AvailableActor : AvailableActorsToLock)
	{
		...
		const FVector PlayerToAvailableTargetNormalized = (AvailableActor->GetActorLocation()-PlayerLocation).GetSafeNormal();
		const FVector CrossResult = FVector::CrossProduct(PlayerToCurrentTargetNormalized,PlayerToAvailableTargetNormalized);
